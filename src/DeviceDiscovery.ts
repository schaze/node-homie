...
export class DeviceDiscovery extends LifecycleBase {
    ...
    public events$ = this._discoveryEvents$.asObservable().pipe(
        ...
        mergeMap((event) =>
            event.pipe(
                delayWhen((event) =>
                    (event.type === "add" && !event.retained) || event.type === "remove"
                        ? timer(this.nonRetainedDiscoveryDelay)
                        : timer(0),
                ),
            ),
        ),
    ...
    public override async onInit() {
        ...
        this.mqtt
            .subscribe(`${rootTopic}/+/${HD_ATTR_STATE}`, true)
    ...
    public makeDevice(id: HomieID): HomieDevice {
        return new HomieDevice(
            { id },
            this.sharedMQTTClient ? this.sharedMQTTClient : this.mqttOpts,
            HomieDeviceMode.Controller,
        );
    }
}
