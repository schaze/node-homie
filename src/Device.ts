...
const DEVICE_ATTRS = ["$homie", "$name", "$state", "$extensions", "$nodes", "$tags"];
...
export class HomieDevice extends HomieRootBase<HomieDeviceAtrributes> {
    ...
    constructor(
        ...
        mode: HomieDeviceMode = HomieDeviceMode.Device,
    ) {
        ...
        if (this.mode === HomieDeviceMode.Device) {
            ...
            this._attributes$.next({
                ...this.attributes,
                homie: HomieDevice.version,
                extensions: HOMIE_EXT_META_4,
                state: "init",
            });
        }
    }

    protected override parseAttribute<T extends keyof HomieDeviceAtrributes>(
        name: T,
        rawValue: string,
    ): HomieDeviceAtrributes[T] {
        switch (name) {
            case "state":
    ...
    public add<T extends HomieNode | undefined = HomieNode>(node: T): T {
        ...
        if (node.parent !== this) {
            throw new Error(
                `Property ${node.id} cannot be added to device ${this.id} as it has a different parent: ${node.parent?.id}`,
            );
        }
    ...
    public async addInitNode<T extends HomieNode | undefined = HomieNode>(node: T): Promise<T> {
        ...
        if (this.isInitialized) {
            // if device is already initialized
            await anode?.onInit(); // initialize node

            if (this.mode === HomieDeviceMode.Device) {
                // if we are in device mode, also publish the nodes attibute
                if (this.attributes.state !== "init") {
                    this.log.warn(
                        `Changed device structure without beeing in 'init' (current state: '${this.attributes.state}') state. This is against specification.`,
                    );
                }
     ...
     */
    public async removeInitNode<T extends HomieNode = HomieNode>(
        node: T | string | null | undefined,
    ): Promise<T | undefined> {
    ...
    public override onItemAttributeMessage<T extends keyof HomieDeviceAtrributes>(name: T, rawValue: string) {
        ...
        if (this.mode === HomieDeviceMode.Controller && name === "nodes") {
            ...
            if (this.attributes.state === "init" && this.isInitialized) {
                // if device is in init mode we also check for removed nodes
    ...
    public deviceChangeTransaction(cb: () => Promise<boolean>): Observable<boolean> {
        return new Observable<boolean>((subscriber) => {
            lastValueFrom(this.updateState$("init"))
                ...
                .finally(() => {
                    lastValueFrom(this.updateState$("ready")).finally(() => {
                        subscriber.complete();
                    });
                });
        });
