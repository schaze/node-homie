...
import { distinctUntilChanged, filter, map, mapTo, pluck, switchMap, take, takeWhile, tap } from "rxjs/operators";
...
export function watchList<T, R>(select: (item: T) => Observable<R>): OperatorFunction<T[], T[]> {
    return (source: Observable<T[]>): Observable<T[]> => {
        return source.pipe(
            switchMap(items => (!items || items.length === 0) ? of<T[]>([]) :
...
export function mergeWatchList<T, S>(predicate: (item: T) => Observable<S[]>): OperatorFunction<T[], S[]> {
    return (source: Observable<T[]>): Observable<S[]> => source.pipe(
        switchMap(list => (!list || list.length === 0) ? of<S[]>([]) :
            combineLatest(
...
export function mergeWatchIndex<T, S>(predicate: (item: T) => Observable<{ [pointer: string]: S }>): OperatorFunction<T[], { [pointer: string]: S }> {
    return (source: Observable<T[]>): Observable<{ [pointer: string]: S }> => source.pipe(
        switchMap(list => (!list || list.length === 0) ? of<{ [pointer: string]: S }>({}) :
            combineLatest(
                list.map(item => predicate(item))
            ).pipe(
...
export function toIndex<T>(getIndex: (item: T) => string): OperatorFunction<T[], { [pointer: string]: T }> {
    return (source: Observable<T[]>): Observable<{ [pointer: string]: T }> => source.pipe(
        map(items => makeIndex(items, getIndex))
    )
}
